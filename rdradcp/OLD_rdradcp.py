...
def checkheader(fd):
    ...
    numbytes = numpy.fromfile(fd, numpy.int16, 1) # Following the header bytes is numbytes    
...
def rd_hdrseg(fd):
    ...
    '''
    hdr = Hdr()
    hdr.nbyte = numpy.fromfile(fd, numpy.int16, 1)
    fd.seek(1, cof)
    ndat = numpy.fromfile(fd, numpy.int8, 1)
...
def rd_hdr(fd):
    ...
    while (cfgid[0] != hex2dec('7F') or cfgid[1] != hex2dec('7F'))  or not checkheader(fd):
        nextbyte = numpy.fromfile(fd, numpy.uint8, 1)
        pos = fd.tell();
        nread = nread + 1
...
def rd_fixseg(fd):
    ...
    cfg.simflag = getopt(numpy.fromfile(fd, numpy.uint8, 1), 'real', 'simulated') # Flag for simulated data
...
def rd_fix(fd):
    ...
    cfgid = numpy.fromfile(fd, numpy.uint16, 1)
    ...
    while k < num_av - 1:
        ...
        for n in range(0, len(hdr.dat_offsets)):
            ...
            # do it here, without an error message to emphasize that I am kludging the WINRIVER blocks only!
                ...
                elif id == '2102': # $xxVTG  (Winriver addition) 45 (but sometimes 46 and 48)
                    cfg.sourceprog = 'WINRIVER';
                    if SOURCE != 2:
                        print('\n***** Apparently a WINRIVER file - Raw NMEA data handler not yet implemented\n\n')
                    #end;
        ...
        offset = (hdr.nbyte[0] + 2) - byte_offset # The 2 is for the checksum
...
def  rdradcp(name, *varargin):
    ...
    if len(nens) == 1:
        ...
        skipb = (hdr.nbyte[0] + 2 + extrabytes) * (nens[0] - 1)
        fd.seek(skipb, cof)
    ...
    for k in range(0, n - 1):
      ...
      # Read an ensemble
        ...
        [ens, hdr, cfg, pos] = rd_buffer(fd, num_av)
        ...
        adcp.corr[:, :, k] = nmean(ens.corr[0], 3)        # added correlation RKD 9/00
        adcp.status[:, :, k] = nmean(ens.status[0], 3)

        adcp.intens[:, :, k] = nmean(ens.intens[0], 3)
        adcp.perc_good[:, :, k] = nmean(ens.percent[0], 3)  # felipe pimenta aug. 2006 - BAD BAD code
        ...
        if cfg.sourceprog == 'WINRIVER':
            adcp.nav_mtime[0, k] = nmean(ens.smtime)
            adcp.nav_longitude[0, k] = nmean(ens.slongitude)
            adcp.nav_latitude[0, k] = nmean(ens.slatitude)
        elif cfg.sourceprog == 'VMDAS':
            adcp.nav_smtime[0, k] = ens.smtime[1]
            adcp.nav_emtime[0, k] = ens.emtime[1]
            adcp.nav_slatitude[0, k] = ens.slatitude[1]
            adcp.nav_elatitude[0, k] = ens.elatitude[1]
            adcp.nav_slongitude[0, k] = ens.slongitude[1]
            adcp.nav_elongitude[0, k] = ens.elongitude[1]
