...
class IndexableSkiplist:
    ...
    def __getitem__(self, i):
        node = self.head
        i += 1
        for level in reversed(list(range(self.maxlevels))):
...
if __name__ == '__main__':
    ...
    for char in excluded:
        print('Attempting to remove:', char, '-->', end=' ')
        try:
    ...
    for i in reversed(list(range(s.maxlevels))):      # show the link structure on each level
        print('Level %d:  ' % i, end=' ')
        line = ''
